...
#'/Users/michaelmuhlbacher/Documents/3_1_Renewables/Power_storage/Erloesoptimierung/beispiel2.csv'
market = pd.read_csv('beispiel1.csv')
...
def loadSell(marketpart, capacity, poweroutput, startload):
    ...
    for i in range(len(sellRegionInKapaPrice)):
        ...
        if (load + kapa * poweroutput <= capacity) & (kapa <= totalPower):
            ...
            totalreturn += sellRegionIndexPrice[idx] * poweroutput * kapa
...
def iterationOverTime(alldata):
    ...
    i = 0
    while i < len(alldata):
        strategy = maximalStrategy(shuffelMarket(alldata[i:i+12]))[3]
        if strategy:
            counter = 0
            if len(strategy[0]) == 1:
                counter += 1
